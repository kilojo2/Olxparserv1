...
Веб-приложение для мониторинга свежих объявлений **долгосрочной аренды квартир** на [OLX.ua](https://www.olx.ua) в заданном городе (по умолчанию — Хмельницкий). Парсит объявления через браузер Playwright, сохраняет в базу данных и показывает на дашборде.
...
- Дедупликация по ID объявления OLX.
...
| `OLX_BASE_URL` | `https://www.olx.ua` | Базовый URL |
...
5. Готово — приложение поднимется, планировщик запустит первый прогон сразу.
...
- Парсер работает с публичными страницами OLX. Учитывайте условия использования сайта и законодательство.
